...
const static long long  MAX_AUDIOQ_SIZE = (16 * 1024);//最大音频包大小（队列中所有加起来的大小）
const static long long   MAX_VIDEOQ_SIZE = (256 * 1024);//最大视频包大小（队列中所有视频包加起来的大小）
...
{
	if (this->audio == nullptr || this->video == nullptr|| this->video->getVideoQueueSize() == -1) {
		return true;
	}
